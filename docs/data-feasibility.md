...
공식 호환 운영 URL `https://api.elections.kalshi.com/trade-api/v2`의 인증 없는 공개
GET으로 KXCPI 현재·과거 시장과 1분 캔들을 다시 조회했다. 기본 운영 URL은 같은
환경에서 HTTP 403 또는 시간 초과가 발생했지만 호환 URL의 재시도 실행은 성공했다.

- 시장 레코드: 과거 483개, 현재 69개
- 결과가 확정된 고정 임계값 `T0.3` 이벤트: 55개
- 계약 종료 전 최근 15분 안에 거래량이 있고 스프레드가 0.10 이하인 이벤트: 29개
- 유효 호가가 오래된 이벤트: 6개
- 조건을 만족한 캔들이 없는 이벤트: 20개

기술적 접근과 표본 존재는 확인됐지만 공개 재배포·상용 이용 허가는 별개다. 상세한
품질 규칙과 확률 성능은 [Kalshi CPI 접근·방법론 검토](kalshi-cpi-access-methods.md)에
기록한다.
